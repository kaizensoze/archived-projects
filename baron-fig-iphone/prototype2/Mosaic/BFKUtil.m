...
+ (void)adjustText:(UIView *)view width:(float)width height:(float)height {
    ...
    attributes = [(NSAttributedString *)((UILabel *)view).attributedText
                  attributesAtIndex:0 effectiveRange:NULL];
    
    CGSize newSize = [BFKUtil textSize:text attributes:attributes width:width height:height];
    CGRect newFrame = CGRectMake(view.frame.origin.x, view.frame.origin.y,
