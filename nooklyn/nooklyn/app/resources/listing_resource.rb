...
class Api::V1::ListingResource < JSONAPI::Resource
  ...
  def fetchable_fields()
    if context && context[:current_user] && context[:current_user].employee?
      [
        :id, :bathrooms, :bedrooms, :featured, :price, :image, :medium_image,
        :primary_thumbnail, :status, :private, :image_updated_at, :hearts_count,
        :updated_at, :latitude, :longitude, :residential, :rental, :description,
        :amenities, :subway_line_url, :subway_line, :station, :neighborhood,
