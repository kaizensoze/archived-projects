function regionInitialize() {
  var mapDiv = document.getElementById('region-show-map');
  if ($(mapDiv).length < 1) {
    return;
  }

  var regionLatitude, regionLongitude;
  regionLatitude = $(mapDiv).data("latitude");
  regionLongitude = $(mapDiv).data("longitude");

  var regionCenter = new google.maps.LatLng(regionLatitude, regionLongitude);
  var mapOptions = {
    zoom: 13,
    center: regionCenter,
    backgroundColor: "#000",
  ...
  var regionId = $(mapDiv).data('region-id');

  $.getJSON('/api/v1/locations?filter[region-id]='+regionId, function(api) {
...
google.maps.event.addDomListener(window, 'load', regionInitialize);
