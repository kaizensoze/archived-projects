function locationInitialize() {
  var mapDiv = document.getElementById('location-show-map');
  if ($(mapDiv).length < 1) {
    return;
  }
  ...
  // page location's marker (the location for the location page we're on)
  var pageLocationMarker = new google.maps.Marker({
  ...
  // use lat/long to get nearby locations/listings
  var params = $.param({
    latitude: listingLat,
  ...
  $.getJSON('/locations.json?' + params, function(api) {
    $.each(api.locations, function(key, data) {
      ...
      // refrain from showing green icon over current page location's marker
      if (data.latitude == listingLat && data.longitude == listingLong) {
