...
function hoodslistingsInitialize() {
  var mapDiv = document.getElementById('nklyn-hood-apartments-map');
  if ($(mapDiv).length < 1) {
    return;
  }
  ...
  $.ajax({
    ...
    success: function(html) {
      ...
      $(".nklyn-listing").each(function() {
        ...
        var listingFormattedprice = $(this).data("formatted-price");
        var listingFormattedbedrooms = $(this).data("formatted-bedrooms");
        var listingFormattedbathrooms = $(this).data("formatted-bathrooms");
  ...
  $(".nooklyn_listings_index_container").on("filterListings", ".nklyn-listing", function(event, filterInfo){
    ...
    function findListingZoningType(listing) {
      var isResidential = $(listing).attr('data-is-residential')
  ...
  $('.filter').on('click', function() {
    ...
    if ($(this).attr('data-filter-type') === "is-residential") {
      $('*[data-filter-type="is-residential"]').removeClass('active');
...
google.maps.event.addDomListener(window, 'load', hoodslistingsInitialize);
